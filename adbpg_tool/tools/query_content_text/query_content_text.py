...
class QueryContentText(Tool):

    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        self.client = AnalyticDBAPIHelper(self.runtime.credentials)
        params = normalize_params(tool_parameters)
        logger.info(f"QueryContentText tool_parameters: {params}")
        response = self.client.query_content_text(**params)
        logger.info(f"QueryContentText response: {response}")
