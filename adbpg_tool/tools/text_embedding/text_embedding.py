...
class TextEmbedding(Tool):

    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        self.client = AnalyticDBAPIHelper(self.runtime.credentials)
        params = normalize_params(tool_parameters)
        logger.info(f"TextEmbedding tool_parameters: {params}")
        response = self.client.text_embedding(**params)
        logger.info(f"TextEmbedding response: {response}")
